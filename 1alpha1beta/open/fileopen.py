...
for i in list:
    x = pd.read_csv('eigenvalues_'+str(i)+'e_1a1b1open.txt',sep='\t',header=None)
    ...
    ax[m,1].set_xlabel(str(i)+' atoms with '+str(int(i/2))+' electron(s)')
    ax[m,1].set_ylabel('Energy Eigenvalue')
    m+=1

plt.savefig('eigenvalues_1a1b1open.png')
